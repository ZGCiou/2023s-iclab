...
reg [3:0] count_out;
reg [inst_sig_width+inst_exp_width:0] p1Reg, p3Reg, p4Reg, p5Reg, acc_uxwh, acc_vy[0:2];//, cal_H[0:2];
reg [3:0] p1Reg_flag;
reg [2:0] p2Reg_flag;
reg p3Reg_flag, p4Reg_flag, p5Reg_flag;
wire [inst_sig_width+inst_exp_width:0] relu_out, sigmoid_out, exp_vh_in, exp_vh_out, add_exp_out, div_out;
// Output
...
always @(posedge clk) begin : proc_data_H
	case (current_state)
		...
		s_cal_1: begin
			if (count == 'd8 || count == 'd17 || count == 'd26) begin
				data_H[0][0] <= data_H[1][0];
			end

			if (count == 'd9 || count == 'd18 || count == 'd27) begin
				data_H[0][1] <= data_H[1][1];
			end

			if (count == 'd10 || count == 'd19 || count == 'd28) begin
				data_H[0][2] <= data_H[1][2];
			end
		end
...
always @(posedge clk or negedge rst_n) begin : proc_cal_count_9
	if (~rst_n)
		...
		case (current_state)
			...
			s_cal_1: begin
				if (cal_count_3 == 2'd2 && cal_count == 2'd2) begin
					if (cal_count_9 == 2'd2)
						cal_count_9 <= 2'd0;
					else
...
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance)
		Add_uxwh(.a(mul_ux_out), .b(mul_wh_out), .rnd(3'b000), .z(add_uxwh_out));
...
always @(posedge clk or negedge rst_n) begin : proc_p1Reg
	if(~rst_n) begin
		...
		if (cal_count == 2'd2)
			p1Reg_flag[0] <= 'd1; //p1Reg_flag[1] -> acc clear
		else
			p1Reg_flag[0] <= 'd0;

		if (cal_count == 2'd2 && cal_count_3 == 2'd2)
			p1Reg_flag[1] <= 'd1; //p1Reg_flag[2] -> data_H updata
		else
			p1Reg_flag[1] <= 'd0;

		p1Reg_flag[3:2] <= cal_count_3;
	end
end
...
// Add_acc_uxwh
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance)
		Add_acc_uxwh(.a(p1Reg), .b(acc_uxwh), .rnd(3'b000), .z(add_acc_uxwh_out));
// Relu
myReLU Fx(.in(add_acc_uxwh_out), .out(relu_out));
...
always @(posedge clk or negedge rst_n) begin : proc_p4Reg
	if(~rst_n) begin
		...
		case (current_state)
			s_idle: p2Reg_flag <= 'd0;
			default : begin
				if (p1Reg_flag[0]) begin
					p2Reg_flag[0] <= 1'b1; 
					p2Reg_flag[2:1] <= p1Reg_flag[3:2];
				end
			end
		endcase	
	end
	...
	case (current_state)
		s_cal_1: begin
			if (p2Reg_flag[0]) begin
				mul_vh_in1 = data_H[1][p2Reg_flag[2:1]];
			end	
			else begin
				mul_vh_in1 = 'd0;
			end		
...
always @(*) begin : proc_mul_vh_in2
	case (current_state)
		s_cal_1: begin
			if (p2Reg_flag[0]) begin
				mul_vh_in2 = weight_V[count_v][p2Reg_flag[2:1]];
			end	
			else begin
				mul_vh_in2 = 'd0;
			end		
...
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance)
		Add_acc_vh(.a(mul_vh_out), .b(add_acc_vh_in), .rnd(3'b000), .z(add_acc_vh_out));
...
always @(posedge clk or negedge rst_n) begin : proc_count_v_3
	if(~rst_n)
		...
		case (current_state)
			...
			default : begin
					if (count_v_3 == 2'd2 && count_v == 2'd2)
						count_v_3 <= 2'd0;
					else if (count_v == 2'd2)
						count_v_3 <= count_v_3 + 'd1;
			end
		endcase		
	end
end
...
// STAGE 4
// Sigmoid -> exp
assign exp_vh_in = {~p3Reg[inst_sig_width+inst_exp_width], p3Reg[inst_sig_width+inst_exp_width-1:0]};
DW_fp_exp #(inst_sig_width, inst_exp_width, inst_ieee_compliance, arch)
	Exp_vh(.a(exp_vh_in), .z(exp_vh_out));
// Pipeline Reg 4
always @(posedge clk or negedge rst_n) begin : proc_p7Reg
	if(~rst_n) begin
		p4Reg <= 'd0;
		p4Reg_flag <= 'd0;
	end 
...
// Sigmoid -> add
DW_fp_add #(inst_sig_width, inst_exp_width, inst_ieee_compliance)
	Add_exp(.a(p4Reg), .b(32'h3F800000), .rnd(3'b000), .z(add_exp_out));
// Pipeline Reg 5
...
DW_fp_div #(inst_sig_width, inst_exp_width, inst_ieee_compliance, faithful_round)
	D0(.a(32'h3F800000), .b(p5Reg), .rnd(3'b000), .z(div_out));
// Pipeline Reg 6(out_Y)
...
// Pipeline Reg 7
always @(posedge clk or negedge rst_n) begin : proc_p8Reg
	if(~rst_n) begin
		p8Reg <= 'd0;
		p8Reg_flag <= 'd0;
	end 
