...
//reg [11:0] quant_coeff;
wire [7:0] quant_result_conv, quant_result_mp_fc;
// Max Pooling
...
always @(posedge G_clock_conv or negedge rst_n) begin : proc_conv_quant_result
	if(~rst_n) begin
		...
		case (cur_state)
			S_CONV_QUANT: begin
				conv_quant_result[cnt_row][cnt_col] <= quant_result_conv;
			end
		endcase
	end
end
...
// Quantiztaion
assign quant_result_conv = conv_result / 'd2295;
assign quant_result_mp_fc = fc_result / 'd510;
/*
// quant_indata
always @(*) begin : proc_quant
	case (cur_state)
		S_CONV_QUANT: begin
			quant_indata = conv_result;
			quant_coeff = 'd2295;
		end

		S_MP_FC_QUANT: begin
			quant_indata = fc_result;
			quant_coeff = 'd510;
		end
...
assign quant_result = quant_indata / quant_coeff;
/*
/*
...
always @(posedge G_clock_mp_fc or negedge rst_n) begin : proc_mp_fc_quant_result
	if(~rst_n) begin
		...
		if (cur_state==S_MP_FC_QUANT) begin
			if (~sel_data)
				mp_fc_quant_result_A[{cnt_row[0], cnt_col[0]}] <= quant_result_mp_fc;
			else
				mp_fc_quant_result_B[{cnt_row[0], cnt_col[0]}] <= quant_result_mp_fc;
		end
	end
end
